...
def load_demo_comments() -> pd.DataFrame:
    ...
    return pd.DataFrame({'comment': ['cảm ơn admin', 'đồ mất dạy', 'ib em nhận báo giá']})
...
def moderation_pipeline(text: str, model_name: str, toxic_threshold: float = 0.7) -> Dict[str, object]:
    ...
    final_label = 'toxic' if toxic_prob >= toxic_threshold else 'clean'
    decision = 'Ẩn tự động' if final_label == 'toxic' else 'Giữ bình luận'
    return {
...
def _plot_topic_distribution(full_df: pd.DataFrame):
    ...
    for bar, value in zip(bars, topic_counts.values):
        ax.text(
            value + max(topic_counts.max() * 0.01, 1),
...
def page_inference():
    ...
    if not model_options:
        ...
        st.warning('Chưa tìm thấy model trong thư mục models/. Hãy chạy notebook Colab trước để sinh model.')

    model_name = st.selectbox('Chọn mô hình', model_options)
    toxic_threshold = st.slider('Ngưỡng toxic để ẩn tự động', min_value=0.50, max_value=0.95, value=0.70, step=0.01)

    default_text = 'ib em nhận bảng giá sỉ và ưu đãi hôm nay nhé'
...
def page_evaluation():
    ...
    if not metric_files:
        st.warning('Chưa có file đánh giá trong thư mục reports/. Hãy chạy notebook Colab để tạo metrics và confusion matrix.')
        return
